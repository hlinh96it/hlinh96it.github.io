...
def fitness_similarity_check(max_fitness, num_similarity):
    similarity = 0
    for n in range(len(max_fitness)):
...
if __name__ == '__main__':
    ...
    def first_generation(pop):
        ...
        return {'individual': population, 'fitness': fitness}
    ...
    finish = False
    while not finish:
        if max(fitness_max) > 6 or max(fitness_avg) > 5 or fitness_similarity_check(fitness_max, 50):
            break
